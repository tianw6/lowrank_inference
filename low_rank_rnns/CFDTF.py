...
def setup():
    ...
    delay_begin = stim_end
    delay_end = delay_begin + delay_duration_discrete

    targets_begin = delay_end
    response_begin = targets_begin
    total_duration = fixation_duration_discrete + stimulus_duration_discrete + delay_duration_discrete + targets_duration_discrete
...
def generate_checker_data(num_trials, coherences=None, std=std_default, fraction_validation_trials=0.2,
    ...
    taskType = np.zeros((num_trials))

    for i in range(num_trials):
        if random.random() > fraction_catch_trials:

            # specify whether a cfd or tf task: cfd 1; tf 2
            cfdortf = random.randint(1, 2)
            taskType[i] = cfdortf
            ...
            if cfdortf == 1: 
                ...
                inputs[i, targets_begin:, 0] += coh_color * SCALE

                if context == 1:
                    inputs[i, stim_begin:, 1] = 1. * SCALE_CTX
                    targets[i, response_begin:] = lo if coh_color > 0 else hi
                elif context == 2:
                    inputs[i, stim_begin:, 2] = 1. * SCALE_CTX
                    targets[i, response_begin:] = hi if coh_color > 0 else lo                
    ...
    taskType_train, taskType_val = taskType[:split_at], taskType[split_at:]


    if fraction_validation_trials > 0:
        return inputs_train, targets_train, mask_train, allCoh_train, taskType_train, inputs_val, targets_val, mask_val, allCoh_val, taskType_val
