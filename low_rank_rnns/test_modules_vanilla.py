...
def train(net, _input, _target, _mask, n_epochs, lr=1e-2, batch_size=32, plot_learning_curve=False, plot_gradient=False, clip_gradient=None, early_stop=None, keep_best=False, cuda=False, resample=False,
          initial_states=None):
    """
    Train a network
    :param net: nn.Module
    ...
    for epoch in range(n_epochs):
        ...
        for i in range(num_examples // batch_size):
            ...
            # loss += 0.001*torch.norm(net.wrec, p=2)/np.sqrt(torch.numel(net.wrec)) + 0.001*torch.norm(h, p=2)/np.sqrt(torch.numel(h))
            ##################################
    ...
    def forward(self, input, return_dynamics=True, initial_states=None):
        ...
        for i in range(seq_len):
            ...
            h = h + self.noise_std * noise[:, i, :] + self.alpha * \
                (-h + r.matmul(self.wrec) + self.b + input[:, i, :].matmul(self.wi_full))
...
class LowRankRNN(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size, noise_std, alpha, rank=1, train_m = True,
                 train_wi=False, train_wo=False, train_wrec=True, train_h0=False, train_si=True, train_so=True,
                 wi_init=None, wo_init=None, m_init=None, n_init=None, wrec_mask = None, wi_mask = None, wo_mask = None, si_init=None, so_init=None, h0_init=None,
                 add_biases=False, non_linearity=torch.relu, output_non_linearity=torch.relu):
        """
        :param input_size: int
        :param hidden_size: int
    ...
    def effective_weight(self, n, m, mask, w_fix=0):
        """ compute the effective weight """
    
        w_eff = n.matmul(m.t()) + w_fix
    ...
    def forward(self, input, return_dynamics=True, initial_states=None):
        ...
        for i in range(seq_len):
            ...
            self.w_rec_eff = self.effective_weight(n=self.n, m = self.m, mask=self.wrec_mask)
    ...
    def clone(self):
        ...
        new_net = LowRankRNN(self.input_size, self.hidden_size, self.output_size, self.noise_std, self.alpha,
                             ...
                             self.train_so, self.wi, self.wo, self.m, self.n, self.wrec_mask, self.wi_mask, self.wo_mask, self.si, self.so, self.h0, False,
                             self.non_linearity, self.output_non_linearity)
