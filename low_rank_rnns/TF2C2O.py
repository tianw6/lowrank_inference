# 2 inputs for color and 2 outputs 
...
def generate_checker_data(num_trials, coherences=None, std=std_default, fraction_validation_trials=0.2,
    ...
    # 2nd input: coh ()
    # 3nd input: red left green right (1 or 0)
    # 4rd input: red right green left (1 or 0)

    # 2 output: choice (
    # 1st output: left (0 to 1)
    # 2nd output: right (0 to 1)
    if coherences is None:
        coherences = [-0.9, -0.6, -0.4, -0.3, -0.2, -0.1, 0.1, 0.2, 0.3, 0.4, 0.6, 0.9]

    # inputs_sensory: red coh of checkerboard
    inputs_sensory = std * torch.randn((num_trials, total_duration, 2), dtype=torch.float32)
    inputs_sensory[:,:stim_begin,:] = 0
    ...
    targets = torch.zeros((num_trials, total_duration, 2), dtype=torch.float32)
    ...
    for i in range(num_trials):
        if random.random() > fraction_catch_trials:
            ...
            inputs[i, stim_begin:stim_end, 1] += -coh_color * SCALE
            ...
            if context == 1:
                ...
                inputs[i, targets_begin:, 3] = -1. * SCALE_CTX

                # desired output
                targets[i, response_begin:,0] = hi if coh_color > 0 else lo
                targets[i, response_begin:,1] = lo if coh_color > 0 else hi
...
def accuracy_checker(targets, output):
    # left: 0; right: 1
    # print(targets[good_trials,110:,0].shape)
    target_decisions = targets[:,110:,0].mean(dim=1) == 0

    leftSum = output[:, 110:, 0].mean(dim=1)
    rightSum = output[:, 110:, 1].mean(dim=1)
    decisions = rightSum > leftSum
