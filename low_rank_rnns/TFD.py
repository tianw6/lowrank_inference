...
def setup():
    ...
    global fixation_duration_discrete, stimulus_duration_discrete, \
        delay_duration_discrete, decision_duration_discrete, total_duration, stim_begin, delay_begin, response_begin, targets_begin
    ...
    delay_begin = targets_begin + targets_duration_discrete
    delay_end = delay_begin + delay_duration_discrete

    stim_begin = delay_end
...
def generate_checker_data(num_trials, coherences=None, std=std_default, fraction_validation_trials=0.2,
    ...
    # 2nd input: context: red left green right -1; red green right red: 1
    ...
    for i in range(num_trials):
        if random.random() > fraction_catch_trials:
            ...
            inputs[i, stim_begin:, 0] += coh_color * SCALE
            if context_spec is None:
                context = random.randint(1, 2)
            else:
                context = context_spec
            if context == 1:
                inputs[i, targets_begin:delay_begin, 1] = 1. * SCALE_CTX
                targets[i, response_begin:] = lo if coh_color > 0 else hi
            elif context == 2:
                inputs[i, targets_begin:delay_begin, 2] = 1. * SCALE_CTX
...
def generate_interrupt_inputs(num_trials, interrupt_type = 'targets'):
    ...
    if interrupt_type == 'targets':
        ...
        interrupt_time = [400, 600, 800, 1000]
